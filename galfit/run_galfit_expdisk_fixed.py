...
def run_galfit(galaxy, outdir, count, inlog_table):
    ...
    #  fix the parameters x,y,mag,radius,b/a,pa
    log_table = pd.read_csv(inlog_table, header=None, sep='\s+')
    x = log_table[1][count]
    y = log_table[2][count]
    mag = log_table[3][count]
    rad = log_table[4][count]
    ba = log_table[5][count]
    pa = log_table[6][count]
    x_y = str(x) + ' ' + str(y) + ' 1'
    ...
    # object 1 is expdisk fix all parameters from single fit
    replace_galfit_param('1', x_y, object_num=1, fixed=True)
    replace_galfit_param('3', mag, object_num=1, fixed=True)
    replace_galfit_param('4', rad, object_num=1, fixed=True)
    replace_galfit_param('9', ba, object_num=1, fixed=True)
    replace_galfit_param('10', pa, object_num=1, fixed=True)

    # object 2 is devauc, let it vary.
    replace_galfit_param('1', x_y, object_num=2, fixed=False)
    replace_galfit_param('3', mag, object_num=2, fixed=False)
    replace_galfit_param('4', rad, object_num=2, fixed=False)
    replace_galfit_param('9', ba, object_num=2, fixed=False)
    replace_galfit_param('10', pa, object_num=2, fixed=False)
...
def main():
    ...
    log_table_f6   = 'log_table_expdish_f6.csv'
    log_table_f8   = 'log_table_expdish_f8.csv'

    # # run galfit for f606w band
    # for count in range(0, 302):
    #     run_galfit('f606w', galfit_outdir, count, log_table_f6)

    # run galfit for f814w band
    for count in range(0, 302):
        run_galfit('f814w', galfit_outdir, count, log_table_f8)
